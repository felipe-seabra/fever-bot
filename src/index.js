...
client.once(Events.ClientReady, (c) => {
  console.log(`Ready! Login performed as ${c.user.tag}`);
});
client.login(TOKEN);

client.on('ready', () => {
  console.log(`${client.user.tag} are you listening`);
  ...
  setInterval(() => client.user.setPresence({
    activities: [{
      name: `${data[(cont + 1) % data.length]}`,
      type: 0,
    }],
  }), 1000 * 30);
  client.user.setStatus('dnd');

  console.log(`${client.user.tag} status: ${client.user.presence.status}`);
});
